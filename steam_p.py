...
high_pressures = range(10, 221, 10)  # 10, 20, ..., 100 bar
...
template = env.get_template("steamtables_ptable.tex")
...
pressures = lowest_pressures + low_pressures + list(medium_pressures) + list(high_pressures)

table_data = []

table_data.append([
    format_value(triple.P * 10, 3),                        # Pressure in bar
...
for pressure in pressures:
    ...
    table_data.append([
        pressure,
        format_value(IAPWS97(P=pressure * 0.1, x=0).T-273.15),
        format_value(IAPWS97(P=pressure * 0.1, x=0).v),
        format_value(IAPWS97(P=pressure * 0.1, x=1).v),
        format_value(IAPWS97(P=pressure * 0.1, x=0).h),
        format_value(IAPWS97(P=pressure * 0.1, x=1).h - IAPWS97(P=pressure * 0.1, x=0).h),
        format_value(IAPWS97(P=pressure * 0.1, x=1).h),
        format_value(IAPWS97(P=pressure * 0.1, x=0).s),
        format_value(IAPWS97(P=pressure * 0.1, x=1).s)
    ])
    
P_crit = 22.064  # MPa
T_crit = 647.096  # K

crit = IAPWS97(P=P_crit, T=T_crit)

table_data.append([
    format_value(P_crit * 10),                        # Pressure in bar
...
table_header_text = f"Saturated steam properties as a function of pressure"
...
tex_filename = "steam_p.tex"
pdf_filename = "steam_p.pdf"
